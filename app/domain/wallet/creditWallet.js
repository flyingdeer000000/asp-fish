...
module.exports = creditWallet = function (app, playerId, gameId, tableId, player, maxFrozenCost) {
...
util.inherits(creditWallet, MemoryWallet);

let proto = creditWallet.prototype;
let cort = P.coroutine;

proto.onInitAfter = function () {
    logger.debug(`pId:${this.playerId}-gId:${this.gameId} creditWallet.onInitAfter `);
...
proto.initAsync = function () {
  ...
  -wagerId:${this.wagerId}-idx:${this.lastIndex} creditWallet.initAsync `);
    ...
    return P.resolve()
        ...
        .catch(err => {
            logger.error(`pId:${this.playerId}-gId:${this.gameId} creditWallet.initAsync error `, err);
...
proto.bet = function (score) {
  ...
  -wagerId:${this.wagerId}-idx:${this.lastIndex} creditWallet.bet `);
...
proto.betResult = function (winScore, ratio, betCash, otherData, cb) {
  ...
  -wagerId:${this.wagerId}-idx:${this.lastIndex}--otherData:${JSON.stringify(otherData)} creditWallet.betResult `);
    let self = this;

    if (self.stoped) {
        logger.error(`pId:${this.playerId}-gId:${this.gameId} creditWallet.betResult error: stoped is true `, {
            winScore,
            ratio,
            betCash
        });
    ...
    if (Math.abs(betCash) > 0 &&
        ...
        logger.info(`[creditWallet][betResult] weapon be changed normal. playerId: ${this.playerId}, shootType: ${otherData.shootType}, betCash: ${betCash}`);
        let bet_res = self.bet(betCash);
        if (!bet_res) {
            logger.info(`[creditWallet][betResult] bet fail. playerId: ${this.playerId}, shootType: ${otherData.shootType}, bet_res: ${bet_res}`);
            return null;
        }
    }
    ...
    if (betCash > 0 || cash > 0) {
        let wagersData = {};
        self.billChecker.betAndWin(self.wagerId, self.lastIndex, betCash, cash, false, wagersData, self._handleBillCallback.bind(this));
    } else {
...
proto._handleBillCallback = function (err, data) {
  ...
  --frozenGain:${this.frozenGain}-frozenCost:${this.frozenCost}
  --error:${!!err} --respone:${util.inspect(data, false, 10)}
  -wagerId:${this.wagerId}-idx:${this.lastIndex} creditWallet._handleBillCallback `);

    let self = this;
    let succ = false;
    const {wagerId, idx} = data;
...
proto.reward = function (cash, conv, ratio, cb) {
  ...
  -wagerId:${this.wagerId}-idx:${this.lastIndex} creditWallet.reward `);

    let self = this;

    if (self.stoped) {
        logger.error(`pId:${this.playerId}-gId:${this.gameId} creditWallet.reward error: stoped is true `, {
            cash,
            ratio
        });
    ...
    self._registerCallBack(self.wagerId, self.lastIndex, cb);
    let wagersData = {};
    self.billChecker.betAndWin(self.wagerId, self.lastIndex, 0, cash, false, wagersData, self._handleBillCallback.bind(this));
...
proto.flushAsync = function () {
  ...
  -wagerId:${this.wagerId}-idx:${this.lastIndex} creditWallet.flushAsync `);
    ...
    return P.resolve()
        .then(() => {
          ...
          --err:${!!err}-dataLen:${!data ? null : data.result.length}
           creditWallet.stopBillChecher return
        `);
            ...
            return new P((resolve, reject) => {
                sync.flush('wallet.batchSave', self.playerId, data, function (err, res) {
                    if (!err) {
                        ...
                        reject(err);
                        logger.error(`pId:${self.playerId}-gId:${self.gameId} creditWallet.flushAsync error `, err);
                    }
                })
            })
        })
...
proto.cancelFreeGain = function (cash, conv, ratio, bills, from, cb) {
  ...
  --from:${from}
  -wagerId:${this.wagerId}-idx:${this.lastIndex} creditWallet.cancelFreeGain `);

    let self = this;

    if (self.stoped) {
        logger.error(`pId:${this.playerId}-gId:${this.gameId} creditWallet.cancelFreeGain error: stoped is true `, {
            cash,
            ratio
        });
    ...
    ++self.lastIndex;
    let cancelRet = self.billChecker.cancel(consts.BillType.betWin, bills);
    ...
    self.billChecker.doHandle(consts.BillType.betWin);
