...
module.exports = clipCreditWallet = function (app, playerId, gameId, tableId, player, clipAmount, reloadAmount) {
...
util.inherits(clipCreditWallet, MemoryWallet);

let proto = clipCreditWallet.prototype;
let cort = P.coroutine;

proto.onInitAfter = function () {
    logger.debug(`pId:${this.playerId}-gId:${this.gameId} clipCreditWallet.onInitAfter `);
...
proto.initAsync = function () {
  ...
  -wagerId:${this.wagerId}-idx:${this.lastIndex} clipCreditWallet.initAsync `);
    ...
    return P.resolve()
        ...
        .catch(err => {
            logger.error(`pId:${this.playerId}-gId:${this.gameId} clipCreditWallet.initAsync error `, err);
...
proto.bet = function (score) {
  ...
  -wagerId:${this.wagerId}-idx:${this.lastIndex} clipCreditWallet.bet `);
...
proto.betResult = function (winScore, ratio, betCash, otherData, cb) {
  ...
  -wagerId:${this.wagerId}-idx:${this.lastIndex}--otherData:${JSON.stringify(otherData)} clipCreditWallet.betResult `);
    let self = this;

    if (self.stoped) {
        logger.error(`pId:${this.playerId}-gId:${this.gameId} clipCreditWallet.betResult error: stoped is true `, {
            winScore,
            ratio,
            betCash
        });
    ...
    if (Math.abs(betCash) > 0 &&
        ...
        logger.info(`[clipCreditWallet][betResult] weapon be changed normal. playerId: ${this.playerId}, shootType: ${otherData.shootType}, betCash: ${betCash}`);
        let bet_res = self.bet(betCash);
        if (!bet_res) {
            logger.info(`[clipCreditWallet][betResult] bet fail. playerId: ${this.playerId}, shootType: ${otherData.shootType}, bet_res: ${bet_res}`);
            return null;
        }
    }
...
proto.getRealBalance = function () {
  ...
  -wagerId:${this.wagerId}-idx:${this.lastIndex} clipCreditWallet.getRealBalance `);
    ...
    if (num < 0) {
        logger.warn(`[clipCreditWallet][getRealBalance] playerId: ${self.playerId}, balance < 0, balance is ${num}, amount: ${self.amount}, gain: ${self.gain}, cost: ${self.cost}, quota: ${self.quota}, ratio: ${self.ratio}`);
        num = 0;
    }
    return num;
}
...
proto.getRealTokens = function () {
  ...
  -wagerId:${this.wagerId}-idx:${this.lastIndex} clipCreditWallet.getRealTokens `);
    ...
    if (amount < 0) {
        logger.warn(`[clipCreditWallet][getRealTokens] playerId: ${self.playerId}, tokens < 0, tokens is ${amount}, amount: ${self.amount}, gain: ${self.gain}, cost: ${self.cost}, quota: ${self.quota}, ratio: ${self.ratio}`);
        amount = 0;
    }
    return amount;
}
...
proto.flushAsync = function () {
  ...
  -wagerId:${this.wagerId}-idx:${this.lastIndex} clipCreditWallet.flushAsync `);
            ...
            return new P((resolve, reject) => {
                sync.flush('wallet.batchSave', self.playerId, data, function (err, res) {
                    if (!err) {
                        ...
                        reject(err);
                        logger.error(`pId:${self.playerId}-gId:${self.gameId} clipCreditWallet.flushAsync error `, err);
                    }
                })
            })
        })
...
proto._rechargeCheckAsync = function () {
  ...
  --wagerId:${this.wagerId}-idx:${this.lastIndex} clipCreditWallet._rechargeCheckAsync `);
    ...
    return P.resolve(0)
        .then(() => {
            return new P((resolve, reject) => {
                let wagersData = {};
                self.billChecker.betAndWin(self.wagerId, self.lastIndex, self.clipAmount, 0, false, wagersData, (err, data) => {
                    logger.debug(`pId:${self.playerId}-gId:${self.gameId}
        --error:${!!err} --respone:${util.inspect(data, false, 10)}
        -wagerId:${self.wagerId}-idx:${self.lastIndex} clipCreditWallet._rechargeCheckAsync `);
    ...
    -wagerId:${self.wagerId}-idx:${self.lastIndex} clipCreditWallet._rechargeCheckAsync error `, err);
...
proto._summaryAsync = function () {
  ...
  --wagerId:${this.wagerId}-idx:${this.lastIndex} clipCreditWallet._summaryAsync `);
    ...
    if (clipBalance < 0) {
        logger.error(`pId:${self.playerId}-gId:${self.gameId}
    -wagerId:${self.wagerId}-idx:${self.lastIndex} clipCreditWallet._summaryAsync error `, err);
    ...
    return P.resolve(0)
        .then(() => {
            return new P((resolve, reject) => {
                let wagersData = {};
                self.billChecker.betAndWin(self.wagerId, self.lastIndex, 0, clipBalance, false, wagersData, (err, data) => {
                    logger.debug(`pId:${self.playerId}-gId:${self.gameId}
        --error:${!!err} --respone:${util.inspect(data, false, 10)}
        -wagerId:${self.wagerId}-idx:${self.lastIndex} clipCreditWallet._summaryAsync `);

                    const {wagerId, idx, winSucc, betSucc} = data;
